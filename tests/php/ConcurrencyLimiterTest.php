...
class ConcurrencyLimiterTest extends WP_UnitTestCase {
	...
	public function test_concurrency_limiting( $backend ): void {
		if ( ! $backend::is_supported() ) {
			static::markTestSkipped( sprintf( 'Backend "%s" is not supported', $backend ) );
		}
		...
		static::assertInstanceOf( $backend, $client1->get_backend() );
		...
		$send_request = function () use ( $es, &$response2, &$send_request, $backend ) {
			...
			static::assertInstanceOf( $backend, $client2->get_backend() );
			$client2->cleanup();
		};
		...
		static::assertIsArray( $response1 );
		static::assertInstanceOf( WP_Error::class, $response2 );
		/** @var WP_Error $response2 */
		static::assertSame( 429, $response2->get_error_code() );
	}
	...
	public function test__get_value( $backend ) {
		...
		$backend->inc_value();
		static::assertSame( 1, $backend->get_value() );
	}
	...
	public function test__index_is_not_limited( $backend ): void {
		...
		static::assertIsArray( $response1 );
		static::assertIsArray( $response2 );
	}

...
class SearchTest extends WP_UnitTestCase {
	...
	public function test__should_load_es_wp_query_already_loaded() {
		require_once __DIR__ . '/../../src/es-wp-query/es-wp-query.php';
	...
	public function test__ep_handle_failed_request__log_message( $response, $expected_message ) {
		...
		$this->search_instance->logger = $this->getMockBuilder( Logger::class )
			->onlyMethods( [ 'log' ] )
			->getMock();
	...
	public function test__ep_indexable_post_types_should_append_attachment_to_array() {
		...
		$features->activate_feature( 'protected_content' );

		$protected_content_feature = $features->get_registered_feature( 'protected_content' );
		$this->assertNotFalse( $protected_content_feature );
		$this->assertTrue( $protected_content_feature->is_active() );
...
function vip_safe_wp_remote_request() {
	return is_null( SearchTest::$mock_global_functions ) ? null : SearchTest::$mock_global_functions->mock_vip_safe_wp_remote_request();
}
...
function wp_remote_request() {
	return is_null( SearchTest::$mock_global_functions ) ? null : SearchTest::$mock_global_functions->mock_wp_remote_request();
}
